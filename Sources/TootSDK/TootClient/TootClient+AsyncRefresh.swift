...
extension TootClient {
    ...
    /// - Returns: An ``_AsyncRefreshResponse`` instance representing the status of the job with the given ID.
    ///
    ...
    public func _getAsyncRefresh(id: String) async throws -> _AsyncRefreshResponse {
        let response = try await _getAsyncRefreshRaw(id: id)
        return response.data
    }

    /// Query the status of a server-side async refresh job.
    /// - Parameter id: The ID of an active async refresh job, provided in the ``TootResponse/asyncRefresh`` property of the response to an API call.
    /// - Returns: A ``TootResponse`` containing the ``_AsyncRefreshResponse`` representing the status of the job with the given ID.
    ///
    /// Expected to return `404` if there is no such job.
    ///
    /// - Warning: TootSDK currently only supports the alpha version of this endpoint as documented at [https://docs.joinmastodon.org/methods/async_refreshes/](https://docs.joinmastodon.org/methods/async_refreshes/).
    public func _getAsyncRefreshRaw(id: String) async throws -> TootResponse<_AsyncRefreshResponse> {
        ...
        return try await fetchRaw(_AsyncRefreshResponse.self, req)
    }
}
...
extension TootFeature {
    ...
    public static let asyncRefreshV1Alpha = TootFeature(requirements: [
        // TODO: When or if this feature moves out of alpha, we will need to add a maxDisplayVersion to this requirement, add an additional TootFeature for the final version, and update `_getAsyncRefreshRaw` to use the appropriate version depending on the server version.
