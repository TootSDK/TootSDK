...
final class AsyncRefreshTests: XCTestCase {
    func testDecodeAsyncRefreshHeaderWithoutResultCount() throws {
        ...
        let result = try decoder.decode(AsyncRefreshHint.self, from: headerContent.utf8Array)
    ...
    func testDecodeAsyncRefreshResponse() throws {
        let json = localContent("mastodon_asyncrefresh")
        let decoder = TootDecoder()

        let result = try decoder.decode(AsyncRefreshResponse.self, from: json)
