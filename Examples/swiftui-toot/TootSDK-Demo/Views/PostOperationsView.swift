    ...
    @ViewBuilder func postButtons(postID: String) -> some View {
        Group {
            ButtonView(text: "Get Post Details") {
                textToShow = try await tootManager.currentClient?.getStatus(id: postID).content?.plainContent ?? "-"
            }
            ...
            ButtonView(text: "Edit post (appends 🧡)") {
                ...
                let editParams = EditStatusParams(status: (oldPost.content?.plainContent ?? "") + "🧡",
